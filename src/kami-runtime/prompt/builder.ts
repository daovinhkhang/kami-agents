...
export const buildSystemPrompt = async (ctx: KamiCtx) => {
  ...
  return [
    ...
    "UI wording: Drafts, reports, approvals, and artifacts are rendered inline as cards in the chat stream or in the current workspace. Do NOT tell the user to look at the right panel, side panel, panel bên phải, or any fixed screen position. Say 'the draft card', 'the report card', 'the action card', or 'the workspace' instead.",
    "",
    ...
    "5. Keep the chat response short; the inline report/artifact card will carry detailed tables and charts.",
